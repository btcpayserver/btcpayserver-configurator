...
public partial class ConfiguratorController
{
        ...
        UpdateSettings<AdditionalServices, AdditionalDataStub> updateSettings)
    {
        ...
        if (ModelState.IsValid)
        {
            ...
            {
                var error = await CheckHostAsync(
                    updateSettings.Settings.LibrePatronSettings.Host, configuratorSettings);
            ...
            {
                var error = await CheckHostAsync(
                    updateSettings.Settings.WooCommerceSettings.Host, configuratorSettings);
                if (!string.IsNullOrEmpty(error))
                {
                    ModelState.AddModelError(
                        nameof(updateSettings.Settings) + "." +
